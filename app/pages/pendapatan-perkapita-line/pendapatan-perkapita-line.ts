...
@Component({
  ...
  templateUrl: 'build/pages/pendapatan-perkapita-line/pendapatan-perkapita-line.html',
})
export class PendapatanPerkapitaLinePage {
  @ViewChild(nvD3) nvD3: nvD3;
  url = "https://api.kawaljakarta.org/v1/pendapatan-perkapita/data&sortBy=tahun&order=asc";
  data;
  options;
  origData;
  dateArr;
  ...
  ngOnInit() {
    this.dataService.load(this.url)
      .then(data => {
        this.origData = data;
        this.keys = ['perkapita_jakarta','perkapita_nasional'];
