...
@Component({
  ...
  templateUrl: 'build/pages/tingkat-inflasi-scatter/tingkat-inflasi-scatter.html',
})
export class TingkatInflasiScatterPage {
  urlData = "https://api.kawaljakarta.org/v1/tingkat-inflasi/data?keterangan=Bukan%20Outlier";
  urlModel = "https://api.kawaljakarta.org/v1/tingkat-inflasi";
  data;
  model;
  modelObj;
  options;
    ...
    this.dataService.load(this.urlModel)
      .then(data => {
        ...
        this.dataService.load(this.urlData)
          .then(data => {
            console.log('@urlData');
            this.model[0].values = this.genScatter(data, "inflasi_jakarta", "inflasi_nasional");
