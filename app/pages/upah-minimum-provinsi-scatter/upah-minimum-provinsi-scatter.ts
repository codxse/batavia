...
export class UpahMinimumProvinsiScatterPage {
  urlData = "https://api.kawaljakarta.org/v1/ump-inflasi/data?keterangan=Bukan%20Outlier";
  urlModel = "https://api.kawaljakarta.org/v1/ump-inflasi/";
  data;
  model;
  modelObj;
  options;
    ...
    this.dataService.load(this.urlModel)
      .then(data => {
        ...
        this.dataService.load(this.urlData)
          .then(data => {
            console.log('@urlData');
            this.model[0].values = this.genScatter(data, "inflasi", "kenaikan_ump");
